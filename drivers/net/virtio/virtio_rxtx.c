			...
			int in_order)
{
	...
	if (use_indirect)
		idx = vq->ring_packed.desc_packed[head_id].index;
			...
			uint16_t nb_pkts)
{
	...
	//rte_prefetch0(&vq->ring_packed.desc_packed[vq->vq_used_cons_idx]);
	//if (!desc_is_used(&vq->ring_packed.desc_packed[vq->vq_used_cons_idx],
	//		  vq))
	//	return 0;
	nb_used = 16;
	...
	vq->head_idx = vq->vq_used_cons_idx;

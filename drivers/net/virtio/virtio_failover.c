 ...
 */
static int hn_vf_attach(struct virtio_hw *hw, uint16_t port_id)
{
	struct rte_eth_dev_owner owner = { .id = RTE_ETH_DEV_NO_OWNER };
	int ret;

/*
	if (hn_vf_attached(hw)) {
...
*/
	...
	ret = rte_eth_dev_owner_set(port_id, &hw->owner);
	if (ret < 0) {
...
int virtio_vf_add(struct rte_eth_dev *dev, struct virtio_hw *hw)
{
	...
	rte_spinlock_lock(&hw->vf_lock);
	err = hn_vf_attach(hw, port);

	if (err == 0) {
		dev->data->dev_flags |= RTE_ETH_DEV_INTR_LSC;
		hw->vf_intr = (struct rte_intr_handle) {
			.fd = -1,
			.type = RTE_INTR_HANDLE_EXT,
		};
		/* Here we change the interrupt handler of the eth_dev to
		 * the vf interrupt handler */
		dev->intr_handle = &hw->vf_intr;
		/* What does this really do? */
		//hn_nvs_set_datapath(hv, NVS_DATAPATH_VF);
	}
	rte_spinlock_unlock(&hw->vf_lock);

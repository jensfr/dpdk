...
/*
			if (vtpci_packed_queue(hw)) {
				uint16_t j;
				uint16_t n = RTE_DIM(txr[i].tx_indir);
				for(j = 0; j < n-1; j++)
					vq->vq_descx[start_dp_packed->index].next = (uint16_t) (j + 1);
				//vq->vq_descx[j].next = VQ_RING_DESC_CHAIN_END;
				//vring_desc_init_packed(start_dp_packed, RTE_DIM(txr[i].tx_indir));
				start_dp_packed->addr = txvq->virtio_net_hdr_mem
					+ i * sizeof(*txr)
					+ offsetof(struct virtio_tx_region, tx_hdr);

				start_dp_packed->len = hw->vtnet_hdr_size;
				start_dp_packed->flags = VRING_DESC_F_NEXT;
			} else {
*/

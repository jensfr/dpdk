	...
	while ((opt = getopt_long(argc, argvopt, SHORTOPTS "ah",
		...
		case 0: /*long options */
			...
			if (!strcmp(lgopts[opt_idx].name, "no-mlockall"))
				do_mlockall = 0;
			if (!strcmp(lgopts[opt_idx].name, "noisy-tx-sw-buffer-size")) {
				n = atoi(optarg);
				if (n > 0)
					noisy_tx_sw_bufsz = (uint16_t) n;
				else
					rte_exit(EXIT_FAILURE,
						 "noisy-tx-sw-buffer-size must be > 0\n");
			}
			if (!strcmp(lgopts[opt_idx].name, "noisy-tx-sw-buffer-flushtime")) {
				n = atoi(optarg);
				if (n >= 0)
					noisy_tx_sw_buf_flush_time = (uint16_t) n;
				else
					rte_exit(EXIT_FAILURE,
						 "noisy-tx-sw-buffer-flushtime must be > 0\n");
			}
			if (!strcmp(lgopts[opt_idx].name, "noisy-lkup-memory")) {
				n = atoi(optarg);
				if (n > 0)
					noisy_lkup_mem_sz = (uint16_t) n;
				else
					rte_exit(EXIT_FAILURE,
						 "noisy-lkup-memory must be > 0\n");
			}
			if (!strcmp(lgopts[opt_idx].name, "noisy-lkup-num-writes")) {
				n = atoi(optarg);
				if (n > 0)
					noisy_lkup_num_writes = (uint16_t) n;
				else
					rte_exit(EXIT_FAILURE,
			...
			if (!strcmp(lgopts[opt_idx].name, "noisy-nb-rnd-read")) {
				n = atoi(optarg);
				if (n > 0)
					noisy_lkup_num_reads = (uint16_t) n;
				else
					rte_exit(EXIT_FAILURE,
						 "noisy-nb-rnd-read must be > 0\n");
			}
			if (!strcmp(lgopts[opt_idx].name, "noisy-lkup-num-reads-writes")) {
				n = atoi(optarg);
				if (n > 0)
					noisy_lkup_num_reads_write = (uint16_t) n;
				else
					rte_exit(EXIT_FAILURE,
